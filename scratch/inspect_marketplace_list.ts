...
async function main() {
  ...
  const matches = findRequestByName(collectionItems, ['getmarketplacelist', 'marketplaces/list']);
  await fs.writeFile('scratch/marketplace_list_endpoint.json', JSON.stringify(matches, null, 2));
  console.log(`Found ${matches.length} matches. Wrote to scratch/marketplace_list_endpoint.json`);
}
