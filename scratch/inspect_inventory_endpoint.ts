...
async function main() {
  ...
  const matches = findRequestByName(collectionItems, ['getinventorydetails']);
  await fs.writeFile('scratch/inventory_endpoint.json', JSON.stringify(matches, null, 2));
  console.log(`Found ${matches.length} matches. Wrote to scratch/inventory_endpoint.json`);
}
